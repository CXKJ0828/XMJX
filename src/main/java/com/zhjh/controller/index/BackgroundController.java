...
//		request.setAttribute("siteInfo", siteList);
	...
	@SystemLog(module="登录",methods="登录验证")//凡需要处理业务逻辑的.都需要记录操作日志
	public String login(String username, String password, String site, HttpServletRequest request) {

//		UserFormMap userFormMap1 = userMapper.findByAttribute("accountName","admin",UserFormMap.class).get(0);
...
//		return null;
			...
			try {
				...
				EmployeeFormMap employeeFormMap=employeeMapper.findbyFrist("id",employeeId,EmployeeFormMap.class);
				if(employeeFormMap!=null){
					String employeeName=employeeFormMap.getStr("name");
                ...
                String role=userFormMap.getStr("roleName");
                if(role!=null&&role.equals("工人")){
					token.clear();
					request.setAttribute("error", "【" + username + "】 为工人账号，只能登录APP!");
					return "/login";
				}else{
...
//				userFormMap.set("accountName", username);
//				userFormMap.set("site", site);
//				List<UserFormMap> userList = baseMapper.findByNames(userFormMap);
//				if(userList == null || userList.size() == 0){
//					request.setAttribute("error", "当前站点不存在此用户");
//					return "/login";
//				}
//			}
...
//			//获取site对象传入到session中
//			SiteFormMap siteClassInfo = siteMapper.findbyFrist("site", request.getParameter("site"), SiteFormMap.class);
//			session.setAttribute("siteMap",siteClassInfo);
//			session.setAttribute("site", siteClassInfo.getStr("site"));
			...
			e.printStackTrace();
			request.setAttribute("error", "登录异常，请联系管理员！提示内容："+e.getMessage());
			return "/login";
		}
