 ...
 */
public class POIReadExcelContainIds {
    private static  Map<String, Object> map[];
    /**
     * 程序入口方法（读取指定位置的excel，将其转换成html形式的字符串，并保存成同名的html文件在相同的目录下，默认带样式）
     * @return <table>...</table> 字符串
     */
    public static String excelWriteToHtml(String sourcePath,int sheetNum,String trs){
        File sourceFile = new File(sourcePath);
        try {
            InputStream fis = new FileInputStream(sourceFile);
            String excelHtml = readExcelToHtml(fis, true,sheetNum,trs);
            return excelHtml;
    ...
    public static String readExcelToHtml(InputStream is,boolean isWithStyle,int sheetNum,String trs){
        String htmlExcel = "";
        try {
    ...
    private static String readWorkbook(Workbook wb,boolean isWithStyle,int sheetNum,String trs){
        String htmlExcel="";
    ...
    public static String getExcelInfo(Workbook wb, boolean isWithStyle,int sheetNum,String trs){
        String excelIdAndTdList="";
        ...
        for(int i=0;i<rows.length;i++){
            ...
            if (null == row) {
                sb.append("<tr id="+rows[i]+"><td >  </td></tr>");
                continue;
