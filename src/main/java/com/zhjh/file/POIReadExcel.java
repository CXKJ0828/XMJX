 ...
 */
import com.zhjh.bean.ExcelIdAndTd;
...
public class POIReadExcel {
    ...
    public static void main(String[] args) {
        String url="C://数据库.xlsx";
        List<ExcelIdAndTd> content=excelWriteToHtml(url,2,"10,12");
    ...
    public static String excelWriteToHtmlWithoprationNameAndState(String sourcePath,int sheetNum,String oporationName,String state,int oprationNum,int stateNum){
        File sourceFile = new File(sourcePath);
        try {
            InputStream fis = new FileInputStream(sourceFile);
            String excelHtml = readExcelToHtml(fis, true,sheetNum,oporationName,state,oprationNum,stateNum);
            return excelHtml;
        } catch (FileNotFoundException e) {
            e.printStackTrace();
            return null;
     ...
     * @return <table>...</table> 字符串
     */
    public static List<ExcelIdAndTd> excelWriteToHtml(String sourcePath,int sheetNum,String trs){
        File sourceFile = new File(sourcePath);
        try {
            InputStream fis = new FileInputStream(sourceFile);
            List<ExcelIdAndTd> excelHtml = readExcelToHtml(fis, true,sheetNum,trs);
            return excelHtml;
        } catch (FileNotFoundException e) {
            e.printStackTrace();
            return null;
    ...
    public static String excelWriteToHtml(String sourcePath,int sheetNum,CountDownLatch downLatch){
        File sourceFile = new File(sourcePath);
        try {
            InputStream fis = new FileInputStream(sourceFile);
            String excelHtml = readExcelToHtml(fis, true,sheetNum);
     ...
     * @return <table>...</table> 字符串
     */
    public static String readExcelToHtml(String filePath , boolean isWithStyle,int sheetNum){
        InputStream is = null;
    ...
    public static String readExcelToHtml(InputStream is,boolean isWithStyle,int sheetNum,String oporationName,String state,int oprationNum,int stateNum){
        String htmlExcel = null;
        try {
            Workbook wb = WorkbookFactory.create(is);
            htmlExcel = readWorkbook(wb,isWithStyle,sheetNum,oporationName,state,oprationNum,stateNum);
        } catch (Exception e) {
            e.printStackTrace();
        }finally{
            try {
    ...
    public static List<ExcelIdAndTd> readExcelToHtml(InputStream is,boolean isWithStyle,int sheetNum,String trs){
        List<ExcelIdAndTd> htmlExcel = new ArrayList<>();
        try {
            Workbook wb = WorkbookFactory.create(is);
            htmlExcel = readWorkbook(wb,isWithStyle,sheetNum,trs);
        } catch (Exception e) {
            e.printStackTrace();
        }finally{
            try {
    ...
    private static String readWorkbook(Workbook wb,boolean isWithStyle,int sheetNum,String oporationName,String state,int oprationNum,int stateNum){
        String htmlExcel = "";
        if (wb instanceof XSSFWorkbook) {
            XSSFWorkbook xWb = (XSSFWorkbook) wb;
            htmlExcel = getExcelInfo(xWb, isWithStyle,sheetNum,oporationName,state,oprationNum,stateNum);
        }else if(wb instanceof HSSFWorkbook){
            HSSFWorkbook hWb = (HSSFWorkbook) wb;
            htmlExcel = getExcelInfo(hWb, isWithStyle,sheetNum,oporationName,state,oprationNum,stateNum);
        }
        return htmlExcel;
    }
    private static List<ExcelIdAndTd> readWorkbook(Workbook wb,boolean isWithStyle,int sheetNum,String trs){
        List<ExcelIdAndTd> htmlExcel=new ArrayList<>();
        if (wb instanceof XSSFWorkbook) {
            XSSFWorkbook xWb = (XSSFWorkbook) wb;
            htmlExcel = getExcelInfo(xWb, isWithStyle,sheetNum,trs);
        }else if(wb instanceof HSSFWorkbook){
            HSSFWorkbook hWb = (HSSFWorkbook) wb;
            htmlExcel = getExcelInfo(hWb, isWithStyle,sheetNum,trs);
        }
        return htmlExcel;
    }
    ...
    public static String getExcelInfo(Workbook wb, boolean isWithStyle,int sheetNum,String oporationName,String state,int oprationNum,int stateNum){
        ...
        for (int rowNum = sheet.getFirstRowNum(); rowNum <= lastRowNum; rowNum ++) {
            ...
            if(rowNum>1){//大于1 进行内容判断
                if(oprationNum<=lastColNum){
                    Cell cellOpration = row.getCell(oprationNum);
                    String oprationNamecontent=getCellValue(cellOpration);

                    Cell cellState = row.getCell(stateNum);
                    String statecontent=getCellValue(cellState);
                    if(statecontent.equals("")){
                        if(oprationNamecontent.contains(oporationName)){
                            sb=sb.append(conniteOpration(sb,lastColNum,sheet,rowNum,row,isWithStyle,wb));
                        }
                    }else{
                        if(oprationNamecontent.equals(oporationName)&&state.equals(statecontent)){
                            sb=sb.append(conniteOpration(sb,lastColNum,sheet,rowNum,row,isWithStyle,wb));
                        }
                    }
                }
...
//
    ...
    public static  StringBuffer conniteOpration( StringBuffer sb,int lastColNum,Sheet sheet,int rowNum,Row row,boolean isWithStyle,Workbook wb){
        sb.append("<tr>");
    ...
    public static String getExcelInfo(Workbook wb,Sheet sheet, boolean isWithStyle,int sheetNum,int start,int end){
    ...
    public static List<ExcelIdAndTd> getExcelInfo(Workbook wb, boolean isWithStyle,int sheetNum,String trs){
        List<ExcelIdAndTd> excelIdAndTdList=new ArrayList<>();
        String[] rows=trs.split(",");
        Sheet sheet = wb.getSheetAt(sheetNum);//获取第一个Sheet的内容
        // map等待存储excel图片
        Map<String, PictureData> sheetIndexPicMap = getSheetPictrues(0, sheet, wb);
        //临时保存位置，正式环境根据部署环境存放其他位置
...
//        sb.append("<table style='border-collapse:collapse;width:100%;'>");
        ...
        for(int i=0;i<rows.length;i++){
            StringBuffer sb = new StringBuffer();
            int rowNum=Integer.parseInt(rows[i]);
            row = sheet.getRow(rowNum);
            int lastColNum = POIReadExcel.getColsOfTable(sheet)[0];
            int rowHeight = POIReadExcel.getColsOfTable(sheet)[1];
            if(null != row) {
...
//            sb.append("<tr>");
            ...
            ExcelIdAndTd  excelIdAndTd=new ExcelIdAndTd(rowNum,sb.toString());
            excelIdAndTdList.add(excelIdAndTd);
            continue;
        }
         return excelIdAndTdList;
     }
     ...
     * @return
     */
    public static Map<String, Object>[] getRowSpanColSpanMap(Sheet sheet) {
    ...
    public static void dealExcelStyle(Workbook wb, Sheet sheet, Cell cell, StringBuffer sb){
        ...
        if (cellStyle != null) {
            ...
            if (wb instanceof XSSFWorkbook) {
                ...
                sb.append("font-size: " + xf.getFontHeight() / 5+ "%;"); // 字体大小
...
//                sb.append("border:solid #000000 1px;");
                ...
                sb.append("font-weight:" + boldWeight + ";"); // 字体加粗
                sb.append("font-size: " + hf.getFontHeight() / 5 + "%;"); // 字体大小
