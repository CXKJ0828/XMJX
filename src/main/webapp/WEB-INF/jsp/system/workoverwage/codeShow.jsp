...
<script>
    ...
    $(function() {
        ...
        code.makeCode("workoverwage:0.5");
    });
    function onChangeDate() {
        alert("aaa");
        duration=getContentBySelect("duration");
        code.makeCode("workoverwage:"+duration);
...
<input id="duration" class="easyui-combobox" data-options="
    valueField: 'value',
    textField: 'text',
    url: '${pageContext.request.contextPath}/json/duration.json',
    onLoadSuccess: function (data) {
       $('#duration').combobox('setValue',data[0].value);//选择后台查出来的第一个值
    },
    onSelect: function(rec){
    code.makeCode('workoverwage:'+rec.value);
    }">
<script>
</script>
