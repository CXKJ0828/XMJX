...
$(function() {
    $('#tt').datagrid({
        url:rootPath + '/unit/findByPage.shtml',
        title:'数据展示',
        iconCls:'icon-ok',
        toolbar: '#tb',
        idField:'id',
...
function  del() {
    ...
    if(ids!=""){
        $.messager.prompt('提示', '请输入删除操作所需密码', function(r){
            if (r){
                communateGet(rootPath +'/unit/deleteEntity.shtml?ids='+ids+'&password='+r,function back(data){
...
function  save() {
    ...
    if(rows.length>0){
        communatePost(rootPath +'/unit/editEntity.shtml',ListToJsonString(rows),function back(data){
            reloadDatagridMessage('tt');
        });
    }else{
